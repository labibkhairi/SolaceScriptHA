...
docker ps -a
docker stop solace-backup
docker rm -f solace-backup
docker ps -a
...
--add-host solace-backup:10.17.101.190 \
--detach=true \
--memory-swap=-1 \
...
--env 'redundancy_matelink_connectvia=solace-primary' \
...
--name=solace-backup solace/solace-pubsub-standard:9.9.0.34
