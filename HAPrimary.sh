...
docker ps -a
docker stop solace-primary
docker rm -f solace-primary
docker ps -a
...
--add-host solace-primary:10.17.101.123 \
--detach=true \
--memory-swap=-1 \
...
--env 'redundancy_matelink_connectvia=solace-backup' \
...
--env 'redundancy_group_node_primary_connectvia=solace-primary' \
--env 'redundancy_group_node_backup_nodetype=message_routing' \
--env 'redundancy_group_node_backup_connectvia=solace-backup' \
--env 'redundancy_group_node_monitor_nodetype=monitoring' \
--env 'redundancy_group_node_monitor_connectvia=solace-monitoring' \
--env 'configsync_enable=yes' \
--name=solace-primary solace/solace-pubsub-standard:9.9.0.34
