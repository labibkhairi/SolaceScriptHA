...
docker ps -a
docker stop solace-monitoring
docker rm -f solace-monitoring
docker ps -a
...
--restart=always \
--add-host solace-monitoring:10.17.101.96 \
--hostname=solaceMonitoring \
--detach=true \
--memory-swap=-1 \
...
--name=solace-monitoring solace/solace-pubsub-standard:9.9.0.34
